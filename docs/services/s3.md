...
account's storage root: an empty name, `.`, `..`, `.accounts`, or a name containing a path
separator is rejected with `InvalidBucketName`. On the persistent and hybrid backends such a name
climbs out of the owning account's directory, and that directory is the account boundary.
